...
export default function TableOfContents({
  ...
  return (
    <div className="space-y-6">
      <div
        className={`px-3 text-xs font-bold uppercase tracking-[0.2em] transition-colors ${
          isLightMode ? "text-black/60" : "text-white/40"
        }`}
      >
        Table of Contents
      </div>
        ...
        {headings.map((h1: any, index: number) => (
          <div key={h1.id} className="relative">
            <a
              ...
              className={`flex items-center gap-3 py-2 px-3 rounded-lg transition-all duration-300 relative z-10 ${
                activeId === h1.id
                  ? isLightMode
            ...
            >
              <span
                className={`text-[10px] font-mono opacity-40 w-4 ${
                  activeId === h1.id ? "opacity-100 text-blue-500" : ""
                }`}
              >
                {activeId === h1.id ? (
                  <Crosshair size={10} />
              ...
              {activeId === h1.id && (
                <motion.div
                  ...
                  className={`absolute inset-0 rounded-lg -z-10 ${
                    isLightMode ? "bg-black/[0.05]" : "bg-white/[0.07]"
                  }`}
            ...
            {h1.children && h1.children.length > 0 && (
              <div
                className={`ml-7 mt-0.5 space-y-0.5 border-l ${
              ...
              >
                {h1.children.map((h2: any) => (
                  <a
                    ...
                    className={`block py-1.5 pl-4 text-[12px] truncate transition-all duration-300 relative ${
                      activeId === h2.id
                        ? isLightMode
